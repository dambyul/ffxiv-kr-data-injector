...
/// <summary>
/// Implementation of the non-standard CRC32 algorithm used by FFXIV for index validation.
/// Derived from FFXIVChnTextPatch (Java) utility classes.
/// </summary>
public static class FFXIVCRC
{
    ...
    {
        uint crc = 0xFFFFFFFF;
        int runningLength = (length / 4) * 4;
        int endUnaligned = length - runningLength;

        for (int i = 0; i < runningLength; i += 4)
        {
            uint val = BinaryPrimitives.ReadUInt32LittleEndian(data.AsSpan(offset + i, 4));
            crc ^= val;
            crc = Table3[crc & 0xFF] ^
                  Table2[(crc >> 8) & 0xFF] ^
                  Table1[(crc >> 16) & 0xFF] ^
        ...
        {
            crc = Table0[(crc ^ data[offset + runningLength + i]) & 0xFF] ^ (crc >> 8);
        }
